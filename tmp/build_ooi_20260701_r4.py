...
SOURCE = ROOT / "tmp/ooi_20260701_r4_table.json"
OUTPUT = ROOT / "tmp/ooi_20260701_r4_model.json"
FINAL_OUTPUT = ROOT / "tmp/ooi_20260701_r4_prediction.json"
    ...
    if result_table is not None:
        for row in result_table.select("tr"):
            ...
            runners[horse_id] = {
                ...
                "body_weight": cells[9],
                "time": str(time_seconds(cells[10])) if re.search(r"\d+:\d{2}\.\d", cells[10]) else "",
                "last_3f": cells[12],
...
def enrich_jra_history(history_rows: list[dict[str, str]]) -> int:
    official = {
        ("フェルメール", "2025-03-23"): ("晴", "https://www.jra.go.jp/JRADB/accessS.html?CNAME=pw01sde0107202502040720250323%2FCE"),
        ("フェルメール", "2025-01-12"): ("曇", "https://www.jra.go.jp/JRADB/accessS.html?CNAME=pw01sde1006202501040820250112%2FDA"),
        ("フェルメール", "2024-11-10"): ("曇", "https://www.jra.go.jp/JRADB/accessS.html?CNAME=pw01sde1005202405040620241110%2F81"),
        ("エーデルワイス", "2025-09-15"): ("晴", "https://www.jra.go.jp/JRADB/accessS.html?CNAME=pw01sde1009202504050420250915%2FF9"),
        ("エーデルワイス", "2025-08-24"): ("晴", "https://www.jra.go.jp/JRADB/accessS.html?CNAME=pw01sde1007202504020220250824%2FC4"),
    }
    filled = 0
    for row in history_rows:
...
def parse() -> tuple[list[dict[str, str]], list[dict[str, object]]]:
    ...
    for start in range(2, len(table), 5):
        ...
        for index, (header, perf, pop_weight) in enumerate(
            ...
            # A race can be an official start/finish record even when the horse was
            # pulled up and no time/sectional/passing order was published. Keep the
            # row instead of silently dropping it; enrichment below fills every
            # field that exists on the official result page.
            if not (date_match and venue_match):
                continue
            ...
            history_rows.append(
                {
                    ...
                    "position": finish,
                    "time": str(time_seconds(perf_match.group(1))) if perf_match else "",
                    ...
                    "last_3f": perf_match.group(3) if perf_match else "",
                    "passing_order": perf_match.group(2).replace("→", "-") if perf_match else "",
...
def main() -> None:
    ...
    final_payload = {
        ...
        "reviewer": {
            "status": "NG",
            "reason": "直近5走70行は取得済みだが、全14頭の当日オッズが未掲載のためEVと買い目を検証できない",
            "fix": "発売開始後に全券種オッズを再取得してev_calculator→bet_builder→reviewerを再実行する",
        },
        "validation": {
            "checked_at": datetime.now(ZoneInfo("Asia/Tokyo")).isoformat(timespec="seconds"),
            "history_rows": len(history_rows),
            "horses": len(horses),
            "history_rows_expected": len(horses) * 5,
            ...
            "availability": {
                ...
                "official_source_blank": [
                    ...
                    f"historical_time:{sum(1 for row in history_rows if not row.get('time'))}",
                    f"historical_last_3f:{sum(1 for row in history_rows if not row.get('last_3f'))}",
                ],
                "history_without_result_link": sum(1 for row in history_rows if not row.get("result_url")),
            },
        },
    }
